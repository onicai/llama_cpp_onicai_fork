...
struct llm_build_context {
    ...
    struct ggml_tensor * build_inp_KQ_mask() {
...
static int llama_decode_internal(
                ...
                case LLAMA_POOLING_TYPE_NONE:
                    {
                        ...
                        if (u_batch.logits) {
                            ...
                            for (uint32_t i = 0; i < n_tokens; i++) {
                                ...
                                ggml_backend_tensor_get_async(backend_embd, embd, embd_out.data() + n_embd*(i + cur_token), (n_embd*i)*sizeof(float), n_embd*sizeof(float));
                            }
                        }
                    } break;
                case LLAMA_POOLING_TYPE_CLS:
                case LLAMA_POOLING_TYPE_MEAN:
                    {
                        ...
                        // add a check and limit max batch size of n_ubatch when pooling?
    ...
    // TODO: fix timings to enable async compute
