...
void ggml_backend_tensor_copy_async(ggml_backend_t backend_src, ggml_backend_t backend_dst, struct ggml_tensor * src, struct ggml_tensor * dst) {
    ...
    // an async copy would normally happen after all the queued operations are completed
...
//#define GGML_MAX_COPIES (GGML_MAX_BACKENDS*32) // FIXME: tune etc
#define GGML_MAX_COPIES 8
    ...
    for (int i = 0; i < sched->n_splits; i++) {
        ...
        for (int j = 0; j < split->n_inputs; j++) {
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                // this copy is queued in the input backend, and split_backend queues a wait until the copy is done
...
ggml_backend_sched_t ggml_backend_sched_new(ggml_backend_t * backends, ggml_backend_buffer_type_t * bufts, int n_backends, size_t graph_size) {
    ...
    // TODO: configurable, only when really needed
    sched->n_copies = 2; //GGML_MAX_COPIES;
...
enum ggml_status  ggml_backend_sched_graph_compute_async(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    // need to review the reset-alloc-compute sequence,
    // and how to allow skipping reset and/or alloc
